...
public class setPassNotes extends BaseActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_set_pass_notes);


        notesPassText = findViewById(R.id.notes_pass_text);
        createBtn = findViewById(R.id.notes_create_btn);
    ...
    private void toFireStore(String pass) throws Exception {
        ...
        fr.collection("Password").document(currentUser.getUid()).collection("Notes Pass").add(model).addOnSuccessListener(new OnSuccessListener<DocumentReference>() {
            @Override
            public void onSuccess(DocumentReference documentReference) {
                finalModel.setDocId(documentReference.getId());
                fr.collection("Password").document(currentUser.getUid()).collection("Notes Pass").document(finalModel.getDocId())
    ...
    public void updateFireStore(String pass) throws Exception {
        fr.collection("Password").document(currentUser.getUid()).collection("Notes Pass").get().addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
            @Override
            public void onComplete(@NonNull Task<QuerySnapshot> task) {
                if(task.isSuccessful() && !task.getResult().isEmpty()){
                    DocumentSnapshot documentSnapshot = task.getResult().getDocuments().get(0);
                    String documentID = documentSnapshot.getId();
                    fr.collection("Password").document(currentUser.getUid()).collection("Notes Pass").document(documentID).delete()
    ...
    public void numOfPass(){
        fr.collection("Password").document(currentUser.getUid()).collection("Notes Pass").count()
                .get(AggregateSource.SERVER).addOnCompleteListener(new OnCompleteListener<AggregateQuerySnapshot>() {
                    ...
                    public void onComplete(@NonNull Task<AggregateQuerySnapshot> task) {
                        if(task.isSuccessful()){
                            ...
                            Log.i("Show number of entries in notes pass", String.valueOf(num));
                        }
                    }
                });
    }
    ...
    class waitTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPreExecute(){
            super.onPreExecute();
            pd = new ProgressDialog(setPassNotes.this);
            pd.show();
        }
        ...
        protected Void doInBackground(Void... voids) {
            ...
            try {
                toFireStore(notesPassText.getText().toString());
            } catch (Exception e) {
                e.printStackTrace();
            }
            try {
